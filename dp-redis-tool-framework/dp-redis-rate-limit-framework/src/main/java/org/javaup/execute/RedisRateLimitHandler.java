...
public class RedisRateLimitHandler implements RateLimitHandler {
    ...
    private void checkBans(Long voucherId, 
        ...
        if (Objects.nonNull(clientIp)) {
            ...
            if (ipBlocked) {
                throw new DpFrameException(BaseCode.SECKILL_RATE_LIMIT_IP_EXCEEDED);
            }
        }
        ...
        if (userBlocked) {
            throw new DpFrameException(BaseCode.SECKILL_RATE_LIMIT_USER_EXCEEDED);
        }
    }
    ...
    private void handleResult(RateLimitContext ctx) {
        ...
        throw new DpFrameException("操作频繁，请稍后再试");
    }
}
