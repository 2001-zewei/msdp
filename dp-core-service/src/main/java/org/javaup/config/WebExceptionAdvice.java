...
public class WebExceptionAdvice {
     ...
     * 业务异常
     * */
    @ExceptionHandler(value = DpFrameException.class)
    public Result<String> toolkitExceptionHandler(HttpServletRequest request, DpFrameException dpFrameException) {
        log.error("业务异常 错误信息 : {} method : {} url : {} query : {} ", dpFrameException.getMessage(), request.getMethod(), getRequestUrl(request), getRequestQuery(request), dpFrameException);
        return Result.fail( dpFrameException.getMessage());
    }
    /**
     * 参数验证异常
     */
