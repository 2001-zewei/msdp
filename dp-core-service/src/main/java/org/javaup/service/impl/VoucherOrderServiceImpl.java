...
import org.javaup.exception.DpFrameException;
...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    public Result<Long> doSeckillVoucherV2(Long voucherId) {
        ...
        if (!seckillVoucherDomain.getCode().equals(BaseCode.SUCCESS.getCode())) {
            throw new DpFrameException(Objects.requireNonNull(BaseCode.getRc(seckillVoucherDomain.getCode())));
        }
    ...
    public void verifyUserLevel(SeckillVoucherFullModel seckillVoucherFullModel,Long userId){
        ...
        if (Objects.isNull(userInfo)) {
            throw new DpFrameException(BaseCode.USER_NOT_EXIST);
        }
        ...
        if (!allowed) {
            throw new DpFrameException("当前会员级别不满足参与条件");
        }
    }
    ...
    public boolean createVoucherOrderV2(MessageExtend<SeckillVoucherMessage> message) {
        ...
        if (Objects.nonNull(normalVoucherOrder)) {
            ...
            throw new DpFrameException(BaseCode.VOUCHER_ORDER_EXIST);
        }
        boolean success = seckillVoucherService.update()
                .setSql("stock = stock - 1")
                ...
                .update();
        if (!success) {
            throw new DpFrameException("优惠券库存不足！优惠券id:" + messageBody.getVoucherId());
        }
    ...
    public Boolean cancel(CancelVoucherOrderDto cancelVoucherOrderDto) {
        ...
        if (Objects.isNull(voucherOrder)) {
            throw new DpFrameException(BaseCode.SECKILL_VOUCHER_ORDER_NOT_EXIST);
        }
        ...
        if (Objects.isNull(seckillVoucher)) {
            throw new DpFrameException(BaseCode.SECKILL_VOUCHER_NOT_EXIST);
        }
